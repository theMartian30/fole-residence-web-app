...
async function showfilter(){
    ...
    var avalue = selectavl.value;
    var aparray;
    if(avalue=="0"){
        aparray = await getRoomsFilter(bvalue, fvalue, false);
    } else {
        aparray = await getRoomsFilter(bvalue, fvalue, true);
    }
...
submitb.addEventListener("click", async function () {
    showfilter();
});
